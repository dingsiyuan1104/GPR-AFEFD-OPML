function  forward=pre_freq_AFEM2d_tri(mesh,pml,freq)
% Form structure for forward modeling with the general PML
% Useage
%    forward=pre_freq_AFEM2d_tri(mesh,pml,freq)
% Input:
...
% Output:
%  forward  --------Structure for forward modeling with the general PML
%
...
sig= mesh.elem(:,5);
omega = 2 * pi *freq;
Sx =  1-(pml.sigx*1i/omega/ep0);
Sy =  1-(pml.sigy*1i/omega/ep0);
A = Sy./Sx;
B = Sx./Sy;
C = Sx.*Sy;
